...
var getTokenDataAction = {
  ...
  handler: async (_agent, input) => {
    try {
      let tokenData;
      if (input.address) {
        tokenData = await getTokenDataByAddress(new PublicKey2(input.address));
      } else if (input.ticker) {
        const address = await getTokenAddressFromTicker(input.ticker);
        if (address) {
          tokenData = await getTokenDataByAddress(new PublicKey2(address));
        }
      }
      if (!tokenData) {
        return {
...
// src/jupiter/actions/cancelLimitOrders.ts
var cancelOrdersSchema = z2.object({
  orders: z2.array(z2.string()).describe("The order public keys to cancel")
});
...
import { signOrSendTX as signOrSendTX2 } from "solana-agent-kit";
async function createLimitOrder(agent, params) {
  ...
  try {
    ...
    const signature = await signOrSendTX2(agent, transaction);
    return {
      signature,
      order: data.order,
      success: true
...
// src/jupiter/actions/createLimitOrder.ts
var createLimitOrderSchema = z3.object({
  inputMint: z3.string(),
  outputMint: z3.string(),
  params: z3.object({
    makingAmount: z3.string(),
    takingAmount: z3.string(),
    expiredAt: z3.string().optional()
  })
});
...
import { signOrSendTX as signOrSendTX3 } from "solana-agent-kit";
async function stakeWithJup(agent, amount) {
  try {
    ...
    const txn = VersionedTransaction2.deserialize(
      Buffer.from(data.transaction, "base64")
    );
    const { blockhash } = await agent.connection.getLatestBlockhash();
    txn.message.recentBlockhash = blockhash;
    return await signOrSendTX3(agent, txn);
  } catch (error) {
    console.error(error);
...
import { signOrSendTX as signOrSendTX4 } from "solana-agent-kit";

// src/jupiter/tools/utils/constants.ts
import { PublicKey as PublicKey3 } from "@solana/web3.js";
var TOKENS = {
  SEND: new PublicKey3("SENDdRQtYMWaQrBroBrJ2Q53fgVuq95CV9UPGEvpCxa"),
  USDC: new PublicKey3("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"),
  SOL: new PublicKey3("So11111111111111111111111111111111111111112")
};
var DEFAULT_OPTIONS = {
  SLIPPAGE_BPS: 300,
  TOKEN_DECIMALS: 9,
...
var METEORA_DYNAMIC_AMM_PROGRAM_ID = new PublicKey3(
  "Eo7WjKq67rjJQSZxS6z3YkapzY3eMj6Xy8X5EQVn5UaB"
);
var METEORA_DLMM_PROGRAM_ID = new PublicKey3(
...
async function trade(agent, outputMint, inputAmount, inputMint = TOKENS.USDC, _slippageBps = DEFAULT_OPTIONS.SLIPPAGE_BPS) {
  try {
    ...
    const inputDecimals = isNativeSol ? 9 : (await getMint(agent.connection, inputMint)).decimals;
    ...
    if (agent.config?.JUPITER_REFERRAL_ACCOUNT) {
      [feeAccount] = PublicKey4.findProgramAddressSync(
        [
          Buffer.from("referral_ata"),
          new PublicKey4(agent.config?.JUPITER_REFERRAL_ACCOUNT).toBuffer(),
          TOKENS.SOL.toBuffer()
        ],
        new PublicKey4(JUP_REFERRAL_ADDRESS)
      );
    }
    ...
    const transaction = VersionedTransaction3.deserialize(swapTransactionBuf);
    const { blockhash } = await agent.connection.getLatestBlockhash();
    transaction.message.recentBlockhash = blockhash;
    return await signOrSendTX4(agent, transaction);
  } catch (error) {
...
var fetchPriceAction = {
  ...
  schema: z4.object({
    tokenAddress: z4.string().describe("The mint address of the token to fetch the price for")
  }),
  handler: async (_agent, input) => {
    try {
      const tokenId = new PublicKey5(input.tokenAddress);
...
var tokenDataByTickerAction = {
  ...
  schema: z7.object({
    ticker: z7.string().min(1).describe("Ticker of the token, e.g. 'USDC'")
  }),
  handler: async (_agent, input) => {
    try {
...
var stakeWithJupAction = {
  ...
  schema: z8.object({
    amount: z8.number().positive().describe("Amount of SOL to stake")
  }),
  handler: async (agent, input) => {
    try {
...
var tradeAction = {
  ...
  schema: z9.object({
    outputMint: z9.string().min(32, "Invalid output mint address"),
    inputAmount: z9.number().positive("Input amount must be positive"),
    inputMint: z9.string().min(32, "Invalid input mint address").optional(),
    slippageBps: z9.number().min(0).max(1e4).optional()
  }),
  handler: async (agent, input) => {
    const tx = await trade(
      agent,
      new PublicKey6(input.outputMint),
      input.inputAmount,
      input.inputMint ? new PublicKey6(input.inputMint) : new PublicKey6("So11111111111111111111111111111111111111112"),
      input.slippageBps
    );
    return {
      status: "success",
...
import { ComputeBudgetProgram, PublicKey as PublicKey7, Transaction } from "@solana/web3.js";
import { signOrSendTX as signOrSendTX5 } from "solana-agent-kit";
...
async function processAll(agent, amount, mint, recipients, priorityFeeInLamports, setupTransaction) {
  ...
  transaction.add(
    ComputeBudgetProgram.setComputeUnitLimit({ units: 5e5 }),
    ComputeBudgetProgram.setComputeUnitPrice({
      microLamports: calculateComputeUnitPrice(priorityFeeInLamports, 5e5)
    })
  );
  ...
  const maxIxs = 3;
  const lookupTableAddress = new PublicKey7(
  ...
  return await signOrSendTX5(agent, tx);
}
...
var compressedAirdropAction = {
  ...
  schema: z10.object({
    mintAddress: z10.string().min(1).describe("Mint address of the token, e.g., 'JUPy...'"),
    amount: z10.number().positive().describe("Number of tokens to airdrop per recipient, e.g., 42"),
    decimals: z10.number().nonnegative().int().describe("Decimals of the token, e.g., 6"),
    recipients: z10.array(z10.string()).nonempty().describe("Array of recipient addresses, e.g., ['1nc1n...']"),
    priorityFeeInLamports: z10.number().optional().describe("Priority fee in lamports (default is 30_000)"),
    shouldLog: z10.boolean().optional().describe("Whether to log progress to stdout (default is false)")
  }),
  handler: async (_agent, input) => {
    try {
      const {
...
import { signOrSendTX as signOrSendTX6 } from "solana-agent-kit";
async function closeEmptyTokenAccounts(agent) {
  try {
    ...
    if (agent.config.signOnly) {
      return {
        signedTransaction: await signOrSendTX6(
          agent,
          transaction
        ),
        size
    ...
    const signature = await signOrSendTX6(agent, transaction);
...
import { SystemProgram, Transaction as Transaction3 } from "@solana/web3.js";
import { LAMPORTS_PER_SOL as LAMPORTS_PER_SOL2 } from "@solana/web3.js";
import { signOrSendTX as signOrSendTX7 } from "solana-agent-kit";
    ...
    if (!mint) {
      const transaction = new Transaction3().add(
        SystemProgram.transfer({
          ...
          toPubkey: to,
          lamports: amount * LAMPORTS_PER_SOL2
        })
      );
      const { blockhash } = await agent.connection.getLatestBlockhash();
      transaction.recentBlockhash = blockhash;
      tx = await signOrSendTX7(agent, transaction.instructions);
    } else {
      const transaction = new Transaction3();
      const fromAta = await getAssociatedTokenAddress2(
        mint,
        agent.wallet.publicKey
      );
      ...
      const mintInfo = await getMint2(agent.connection, mint);
...
async function get_balance(agent, token_address) {
  if (!token_address) {
    return await agent.connection.getBalance(agent.wallet.publicKey) / LAMPORTS_PER_SOL3;
  }
...
import {
  LAMPORTS_PER_SOL as LAMPORTS_PER_SOL4
} from "@solana/web3.js";
async function get_balance_other(agent, wallet_address, token_address) {
  try {
    if (!token_address) {
      return await agent.connection.getBalance(wallet_address) / LAMPORTS_PER_SOL4;
    }
...
async function getTokenMetadata(connection, tokenMint) {
  const METADATA_PROGRAM_ID = new PublicKey9(
    "metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s"
  );
  const [metadataPDA] = PublicKey9.findProgramAddressSync(
    [
      Buffer.from("metadata"),
      METADATA_PROGRAM_ID.toBuffer(),
      new PublicKey9(tokenMint).toBuffer()
    ],
    METADATA_PROGRAM_ID
  );
  ...
  if (data[offset] === 1) {
    ...
    creators = [...Array(numCreators)].map(() => {
      const creator = {
        address: new PublicKey9(data.slice(offset, offset + 32)),
...
async function get_token_balance(agent, walletAddress) {
  ...
  const solBalance = lamportsBalance / LAMPORTS_PER_SOL5;
  return {
    sol: solBalance,
...
var balanceAction = {
  ...
  schema: z11.object({
    tokenAddress: z11.string().optional()
  }),
  handler: async (agent, input) => {
    const balance = await get_balance(
      agent,
      input.tokenAddress && new PublicKey10(input.tokenAddress)
    );
    return {
      status: "success",
      balance,
...
var tokenBalancesAction = {
  ...
  schema: z15.object({
    walletAddress: z15.string().optional()
  }),
  handler: async (agent, input) => {
    const balance = await get_token_balance(
      agent,
      input.walletAddress ? new PublicKey11(input.walletAddress) : void 0
    );
    return {
      status: "success",
      balance
...
var transferAction = {
  ...
  schema: z16.object({
    to: z16.string().min(32, "Invalid Solana address"),
    amount: z16.number().positive("Amount must be positive"),
    mint: z16.string().optional()
  }),
  handler: async (agent, input) => {
    const recipient = new PublicKey12(input.to);
    const mintAddress = input.mint ? new PublicKey12(input.mint) : void 0;
...
async function findTokenContract(symbol, chain) {
  const tokens = await fetchTokenList(chain, true);
...
var swapAction = {
  ...
  schema: z18.object({
    ...
    toToken: z18.string(),
    dstAddr: z18.string().min(32, "Invalid destination address"),
    inputAmount: z18.number().positive("Input amount must be positive"),
    slippageBps: z18.number().min(0).max(1e4).optional()
  }),
  handler: async (agent, input) => {
...
import { Keypair, VersionedTransaction as VersionedTransaction4 } from "@solana/web3.js";
import {
  signOrSendTX as signOrSendTX8
...
async function launchPumpFunToken(agent, tokenName, tokenTicker, description, imageUrl, options) {
  try {
    ...
    const tx = VersionedTransaction4.deserialize(
    ...
    return {
      signature: await signOrSendTX8(agent, tx, [mintKeypair]),
      mint: mintKeypair.publicKey.toBase58(),
      metadataUri: metadataResponse.metadataUri
    };
  } catch (error) {
...
var launchPumpfunTokenAction = {
  ...
  schema: z19.object({
    tokenName: z19.string().min(1).max(32).describe("Name of the token"),
    tokenTicker: z19.string().min(2).max(10).describe("Ticker symbol of the token"),
    description: z19.string().min(1).max(1e3).describe("Description of the token"),
    imageUrl: z19.string().url().describe("URL of the token image"),
    twitter: z19.string().optional().describe("Twitter handle (optional)"),
    telegram: z19.string().optional().describe("Telegram group link (optional)"),
    website: z19.string().url().optional().describe("Website URL (optional)"),
    initialLiquiditySOL: z19.number().min(1e-4).default(1e-4).describe("Initial liquidity in SOL"),
    slippageBps: z19.number().min(1).max(1e3).default(5).describe("Slippage tolerance in basis points"),
    priorityFee: z19.number().min(1e-5).default(5e-5).describe("Priority fee in SOL")
  }),
  handler: async (agent, input) => {
    try {
...
var pythFetchPriceAction = {
  ...
  schema: z20.object({
    tokenSymbol: z20.string().min(1).describe("The token symbol to fetch the price for")
  }),
  handler: async (_agent, input) => {
    try {
...
var rugcheckAction = {
  ...
  schema: z21.object({
    mint: z21.string().min(1).describe("The token mint address")
  }),
  handler: async (_agent, input) => {
    try {
...
import { signOrSendTX as signOrSendTX9 } from "solana-agent-kit";
  ...
  try {
    ...
    for (const transaction of versionedTxns) {
      try {
        const { blockhash } = await agent.connection.getLatestBlockhash();
        transaction.message.recentBlockhash = blockhash;
        const signatureOrTx = await signOrSendTX9(agent, transaction);
  ...
  try {
    ...
    for (const txn of swapData.transactions) {
      try {
        ...
        const signatureOrTx = await signOrSendTX9(agent, txn.transaction);
        signaturesOrTxs.push(signatureOrTx);
      } catch (_error) {
        continue;
      }
...
var burnTokensAction = {
  ...
  schema: z22.object({
    mints: z22.array(z22.string()).describe("Array of mint addresses to burn")
  }),
  handler: async (agent, input) => {
    try {
      const { mints } = input;
...
var closeAccountsAction = {
  ...
  schema: z23.object({
    mints: z23.array(z23.string()).describe("Array of mint addresses to close")
  }),
  handler: async (agent, input) => {
    try {
      const { mints } = input;
...
var mergeTokensAction = {
  ...
  schema: z24.object({
    inputAssets: z24.array(
      z24.object({
        mint: z24.string(),
        amount: z24.string()
      })
    ).describe("Array of input assets to merge"),
    outputMint: z24.string().describe("Output token mint address"),
    priorityFee: z24.enum(["fast", "turbo", "ultra"]).describe("Transaction priority level")
  }),
  handler: async (agent, input) => {
    try {
...
var spreadTokenAction = {
  ...
  schema: z25.object({
    inputAsset: z25.object({
      mint: z25.string(),
      amount: z25.string()
    }).describe("Input asset to spread"),
    targetTokens: z25.array(
      z25.object({
        mint: z25.string(),
        percentage: z25.number()
      })
    ).describe("Target tokens with allocation percentages"),
    priorityFee: z25.enum(["fast", "turbo", "ultra"]).describe("Transaction priority level")
  }),
  handler: async (agent, input) => {
    try {
